...
class RandCrop(object):
    ...
    def __call__(self, sample):
        ...
        sample = {'r_img': r_img, 'd_img': d_img, 'score': score, 'mscn_r_img': sample['mscn_r_img'], 'mscn_d_img': sample['mscn_d_img']}
        return sample
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        r_img, d_img = sample['r_img'], sample['d_img']
        mscn_r_img= sample['mscn_r_img']
        mscn_d_img= sample['mscn_d_img']
        score = sample['score']
        
        r_img = torch.from_numpy(r_img)
        d_img = torch.from_numpy(d_img)
        mscn_d_img = torch.from_numpy(mscn_d_img)
        mscn_r_img = torch.from_numpy(mscn_r_img)
