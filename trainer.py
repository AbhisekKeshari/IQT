...
def train_epoch(config, epoch, model_transformer, model_backbone, save_output, criterion, optimizer, scheduler, train_loader):
    ...
    #config.n_enc_seq = 22*22
    #config.n_dec_seq = 22*22
    ...
    for data in tqdm(train_loader):
        ...
        mscn_r_img, mscn_d_img = data['mscn_r_img'].to(config.device), data['mscn_d_img'].to(config.device)
        labels = data['score']
        ...
        #feat_ref = torch.flatten(feat_ref, start_dim=1)
        #feat_dis = torch.flatten(feat_dis, start_dim=1)
        #feat_ref = torch.cat((feat_ref, mscn_r_img), 1)
        #feat_dis = torch.cat((feat_dis, mscn_d_img), 1)
        feat_diff = feat_ref - feat_dis
        '''temp = torch.mean(feat_diff,dim=1)
        zeros_tensor = torch.zeros(16,82524).to(config.device)
        for i in range(0,16):
            zeros_tensor[i]=temp[i]
        feat_diff = torch.cat((feat_diff, zeros_tensor), 1)

        temp = torch.mean(feat_ref, dim=1)
        zeros_tensor = torch.zeros(16, 82524).to(config.device)
        for i in range(0, 16):
            zeros_tensor[i] = temp[i]
        feat_ref = torch.cat((feat_ref, zeros_tensor), 1)
        b, dim = feat_diff.size()
        feat_diff= torch.reshape(feat_diff,(b, 1920, 22,22))
        feat_ref = torch.reshape(feat_ref, (b, 1920, 22, 22))'''

        enc_inputs_embed = feat_diff
        dec_inputs_embed = feat_ref
        # weight update
        dec_inputs_embed = dec_inputs_embed.float()
        enc_inputs_embed = enc_inputs_embed.float()
        optimizer.zero_grad()
